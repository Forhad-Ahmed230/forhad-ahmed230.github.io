...
const CACHE_VERSION = 'shapla-art-v1';
...
self.addEventListener('install', function(event) {
  ...
  event.waitUntil(
    caches.open(CACHE_VERSION).then(function(cache) {
      ...
      // প্রতিটি আলাদাভাবে — একটি ব্যর্থ হলেও বাকি cache হবে
...
self.addEventListener('fetch', function(event) {
  ...
  // Firebase Firestore/Auth রিকোয়েস্ট — সরাসরি নেটওয়ার্কে যাবে
  if (
    ...
    return; // SW bypass — Firebase নিজে handle করবে
  }
  ...
  event.respondWith(
    caches.match(event.request).then(function(cachedResponse) {
      if (cachedResponse) {
        // Cache থেকে দিলাম, পাশাপাশি background-এ আপডেট
...
// ── Push Notification (ভবিষ্যতের জন্য প্রস্তুত) ──
